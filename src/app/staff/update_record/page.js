...
import { fetchStaffMemberData, fetchStudentData } from "../services";
import { fetchStaffData } from "@/app/view_summary/services";
...
export default function NewRecordStudents() {
  ...
  useEffect(() => {
    ...
    if (staffData?.ailment === undefined) {
      setTimeout(async () => {
        const staff_data = await fetchStaffMemberData(id_number);
  ...
  const formik = useFormik({
    ...
    initialValues: {
      idNo: idNo,
      tempReading: staffData?.tempReading,
      complain: staffData?.complain,
      ailment: staffData?.ailment,
      medication: staffData?.medication,
    },
